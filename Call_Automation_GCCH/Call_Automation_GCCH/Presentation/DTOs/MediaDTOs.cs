using System.ComponentModel.DataAnnotations;

namespace Call_Automation_GCCH.Presentation.DTOs;

/// <summary>
/// Request DTOs for Media Controller (Clean Architecture)
/// </summary>

public class PlayMediaRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    [Required] public string TargetParticipant { get; set; } = string.Empty;
    public bool IsPstn { get; set; }
    public string? AudioFileUrl { get; set; }
    public string? TextToPlay { get; set; }
    public bool Loop { get; set; }
    public bool InterruptCallMediaOperation { get; set; }
    public string? OperationContext { get; set; }
}

public class PlayToAllRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    public string? AudioFileUrl { get; set; }
    public string? TextToPlay { get; set; }
    public bool Loop { get; set; }
    public bool InterruptCallMediaOperation { get; set; }
    public string? OperationContext { get; set; }
}

public class MediaHoldRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    [Required] public string ParticipantId { get; set; } = string.Empty;
    public bool IsPstn { get; set; }
    public string? PlaySourceId { get; set; }
    public string? OperationContext { get; set; }
}

public class MediaUnholdRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    [Required] public string ParticipantId { get; set; } = string.Empty;
    public bool IsPstn { get; set; }
    public string? OperationContext { get; set; }
}

public class MediaStreamingRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    public string? OperationContext { get; set; }
}

public class CancelMediaRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
}

public class TranscriptionRequest
{
    [Required] public string CallConnectionId { get; set; } = string.Empty;
    public string? Locale { get; set; }
    public string? OperationContext { get; set; }
}
