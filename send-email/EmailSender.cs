    ...
    class EmailSender
    {
        ...
        public async Task SendEmailToRecipients(EmailTemplate email)
        {
            ...
            try
            {
                ...
                var emailImportance = EmailImportance.Normal;
                if (!string.IsNullOrWhiteSpace(email.Importance))
                {
                    emailImportance = new EmailImportance(email.Importance.ToLower());
                }
                emailMessage.Importance = emailImportance;
                
                // email client for sending email
        ...
        public async Task CheckEmailStatus(Response response, string emailName)
        {
            try
            {
                ...
                Response<SendStatusResult> sendStatus = null;

                DateTime checkStatusStartTime = DateTime.Now.ToLocalTime();

                do
                {
                    sendStatus = this.emailClient.GetSendStatus(messageId);

                    Console.WriteLine($"[{DateTime.Now.ToLocalTime()}] Email send status for {emailName} :: [{sendStatus.Value.Status}],  waitTime = {DateTime.Now.ToLocalTime() - checkStatusStartTime} secs");
