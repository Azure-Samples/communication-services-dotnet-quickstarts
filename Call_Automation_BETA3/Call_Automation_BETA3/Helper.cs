                ...
                while (!receiveResult.CloseStatus.HasValue)
                {
                    ...
                    if (response != null)
                    {
                        ...
                        if (response is TranscriptionMetadata transcriptionMetadata)
                        {
                            ...
                            //Console.WriteLine("ENABLE SENTIMENT ANALYSIS-->" + transcriptionMetadata?.IsSentimentAnalysisEnabled);
                            //Console.WriteLine("SPEECH RECOGNITION MODEL ENDPOINT-->" + transcriptionMetadata?.SpeechRecognitionModelEndpointId);
                            //Console.WriteLine("PII REDACTION ENABLED-->" + transcriptionMetadata?.PiiRedactionOptions?.IsEnabled);
                            //Console.WriteLine("REDACTION TYPE-->" + transcriptionMetadata?.PiiRedactionOptions?.RedactionType);

                            //if (transcriptionMetadata?.Locales != null && transcriptionMetadata?.Locales.Count > 0)
                            //{
