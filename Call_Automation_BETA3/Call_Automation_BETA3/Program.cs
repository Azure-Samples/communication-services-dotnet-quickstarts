...
string fileSourceUri = "https://waferwireteststorage.z3.web.core.usgovcloudapi.net/recording-file.wav";
...
//CallAutomationClient client = new CallAutomationClient(new Uri("https://nextpma-us.plat.skype.com:6448"), acsConnectionString);

//CallAutomationClient client = new CallAutomationClient(new Uri("https://uswc-01.sdf.pma.teams.microsoft.com"), acsConnectionString);
//var credential = new DefaultAzureCredential();
//CallAutomationClient client = new CallAutomationClient(new Uri("https://uswc-01.sdf.pma.teams.microsoft.com"), new Uri("https://dacsrecordingtest.unitedstates.communication.azure.com"), credential);

//https://nextpma.plat.skype.com:6448
CallAutomationClient client = new CallAutomationClient(new Uri("https://nextpma.plat.skype.com:6448"),acsConnectionString);
    ...
    foreach (var eventGridEvent in eventGridEvents)
    {
        ...
        if (eventGridEvent.TryGetSystemEventData(out object eventData))
        {
            ...
            if (eventData is AcsIncomingCallEventData incomingCallEventData)
            {
                ...
                MediaStreamingOptions mediaStreamingOptions = new MediaStreamingOptions(
                    new Uri(websocketUri), MediaStreamingContent.Audio, MediaStreamingAudioChannel.Unmixed, MediaStreamingTransport.Websocket,false);

                TranscriptionOptions transcriptionOptions = new TranscriptionOptions(
                   new Uri(websocketUri), TranscriptionTransport.Websocket, "en-US", false);
...
app.MapPost("/api/callbacks", (CloudEvent[] cloudEvents, ILogger<Program> logger) =>
{
    foreach (var cloudEvent in cloudEvents)
    {
        ...
        //    var transcriptionUpdate = transcriptionCallSummaryUpdate?.TranscriptionUpdate;
        //    var msg = transcriptionUpdate?.TranscriptionMessage;
        //    logger.LogInformation($"Transcription update message: {msg}");
        ...
        else if (parsedEvent is PostDialDtmfTonesCompleted postDialDtmfTonesCompleted)
        {
            logger.LogInformation($"Received call event: {postDialDtmfTonesCompleted.GetType()}");
            callConnectionId = postDialDtmfTonesCompleted.CallConnectionId;
        }
        else if (parsedEvent is PostDialDtmfTonesFailed postDialDtmfTonesFailed)
        {
            callConnectionId = postDialDtmfTonesFailed.CallConnectionId;
            logger.LogInformation($"Received call event: {postDialDtmfTonesFailed.GetType()}, CorrelationId: {postDialDtmfTonesFailed.CorrelationId}, " +
                      $"subCode: {postDialDtmfTonesFailed.ResultInformation?.SubCode}, message: {postDialDtmfTonesFailed.ResultInformation?.Message}, context: {postDialDtmfTonesFailed.OperationContext}");
        }
    }
    return Results.Ok();
...
app.MapPost("/outboundCallToPstnAsync", async (string targetPhoneNumber, ILogger<Program> logger) =>
{
    ...
    {
        CallIntelligenceOptions = new CallIntelligenceOptions() { CognitiveServicesEndpoint = new Uri(cognitiveServicesEndpoint) },
        //PostDialTones = "1234567890123456789012345678901", // Example DTMF tones to be sent after call is connected
    };
...
app.MapPost("/outboundCallToAcsAsync", async (string acsTarget, ILogger<Program> logger) =>
{
    ...
    //TeamsPhoneCallerDetails teamsPhoneCallerDetails = new TeamsPhoneCallerDetails(new CommunicationUserIdentifier("8:acs"),"TestName","+1234567890")
    //{
    //    RecordId = "RecordId_12345",
    //};

    //TeamsPhoneSourceDetails teamsPhoneSourceDetails = new TeamsPhoneSourceDetails(new CommunicationUserIdentifier("8:acs"), "en-us", "active");
    //{
    ...
    //   CallTopic= "Test Topic",
    //   CallContext = "TestContext"
    //};

    callInvite.CustomCallingContext.AddVoip("CustomVoipKey", "CustomVoipValue");
...
// 8:acs:4c9e9161-d7b9-4429-9745-16b8257e01b1_c28ea9b0-aa11-4d0b-9a67-cfa2aab689b0_de94f7e7-8457-4762-b780-4715ada6bac9
// ResourceID: 4c9e9161-d7b9-4429-9745-16b8257e01b1te
// TenantId: c28ea9b0-aa11-4d0b-9a67-cfa2aab689b0
// UserId: de94f7e7-8457-4762-b780-4715ada6bac9
app.MapPost("/addTPEParticipantAsync", async (string resourceId, string tenantId, string userId, ILogger <Program> logger) =>
{
    CallConnection callConnection = GetConnection();
    TeamsExtensionUserIdentifier teamsExtensionUserIdentifier = new TeamsExtensionUserIdentifier(userId: userId, tenantId: tenantId, resourceId: resourceId);
    
    CallInvite callInvite = new CallInvite(teamsExtensionUserIdentifier);
    var addParticipantOptions = new AddParticipantOptions(callInvite)
    {
        OperationContext = "addTPEUserContext",
        InvitationTimeoutInSeconds = 30,
    };
...
app.MapPost("/addPstnParticipantInOPSCallAsync", async (string pstnParticipant, ILogger<Program> logger) =>
{
    CallConnection callConnection = GetConnection();
    CallInvite callInvite = new CallInvite(new PhoneNumberIdentifier(pstnParticipant),
           null);
...
app.MapPost("/playMediaToAllAsync", async (bool isMultipleSource, ILogger<Program> logger) =>
{
    ...
    {
        FileSource fileSource = new FileSource(new Uri(fileSourceUri));
        playToAllOptions = new(playSource: fileSource)
        {
            OperationContext = "playToAllContext"
        };
    }
...
app.MapPost("/createCallToPstnWithMediaStreamingAsync", async (string targetPhoneNumber, bool isEnableBidirectional, bool isPcm24kMono, bool isMixed, bool isStartMediaStreaming, ILogger<Program> logger) =>
{
    ...
    MediaStreamingOptions mediaStreamingOptions = new MediaStreamingOptions(
                    new Uri(websocketUri), MediaStreamingContent.Audio, isMixed ? MediaStreamingAudioChannel.Mixed : MediaStreamingAudioChannel.Unmixed, MediaStreamingTransport.Websocket, isStartMediaStreaming);
    mediaStreamingOptions.EnableBidirectional = isEnableBidirectional;
    mediaStreamingOptions.AudioFormat = isPcm24kMono ? AudioFormat.Pcm24KMono : AudioFormat.Pcm16KMono;
    mediaStreamingOptions.EnableDtmfTones = true;
...
app.MapPost("/createCallToPstnWithTranscriptionAsync", async (string targetPhoneNumber, bool isStartTranscription, ILogger<Program> logger) =>
{
    ...
    //TranscriptionOptions transcriptionOptions = new TranscriptionOptions("es-ES");
    //transcriptionOptions.TransportUri = new Uri(websocketUri);
    //transcriptionOptions.EnableIntermediateResults = false;
    //transcriptionOptions.StartTranscription = isStartTranscription;
    //transcriptionOptions.IsSentimentAnalysisEnabled = true;
    //transcriptionOptions.PiiRedactionOptions = new PiiRedactionOptions()
    //{
    //    IsEnabled = true,
    //    RedactionType = RedactionType.MaskWithCharacter
    //};
    //transcriptionOptions.Locales = new List<string> { "es-ES", "en-US" };
...
app.MapPost("/createCallToPstnWithTranscription", (string targetPhoneNumber, bool isStartTranscription, ILogger<Program> logger) =>
{
    ...
    TranscriptionOptions transcriptionOptions = new TranscriptionOptions(
       new Uri(websocketUri), TranscriptionTransport.Websocket, "en-US", isStartTranscription);
...
app.MapPost("/createCallToAcsWithTranscriptionAsync", async (string acsTarget, bool isStartTranscription, ILogger<Program> logger) =>
{
    ...
    //    RedactionType = RedactionType.MaskWithCharacter
    //};
    //transcriptionOptions.Locales = new List<string> { "en-US", "es-ES" };
    //transcriptionOptions.SummarizationOptions = new SummarizationOptions()
    //{
    //    Locale = "en-US",
    //    IsEndCallSummaryEnabled = true
...
{
    //UpdateTranscriptionOptions updateTranscriptionOptions = new UpdateTranscriptionOptions("en-US")
    //{
    ...
    CallMedia callMedia = GetCallMedia();
    await callMedia.UpdateTranscriptionAsync("en-US");
    return Results.Ok();
}).WithTags("Transcription APIs");
...
app.MapPost("/startTranscriptionWithOptionsAsync", async (ILogger<Program> logger) =>
{
    ...
    StartTranscriptionOptions startTranscriptionOptions = new StartTranscriptionOptions()
    {
        ...
        //},
        //IsSentimentAnalysisEnabled = true,
        //Locales = new List<string> { "en-US", "hi-IN" },
        //SummarizationOptions = new SummarizationOptions()
        //{
        //    Locale = "en-US",
...
//app.MapPost("/summarizeCall", (ILogger<Program> logger) =>
//{
//    CallMedia callMedia = GetCallMedia();
...
//        }
//    };
//    callMedia.SummarizeCallAsync(summarizeCallOptions);
